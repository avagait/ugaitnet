...
def mj_computeDistMetrics(model, val_generator, multitask, lab4color, gaitset=False):
    ...
    for bix in range(len(val_generator)):
        ...
        of0 = tuples
        codes0 = UWYHSemiNet_BothDatasets.encode(model, of0, [1.0, 1.0], gaitset)
...
def trainUWYHGaitNet(datadir="matimdbtum_gaid_N150_of25_60x60_lite", dbbasedir='/home/GAIT_local/SSD', experfix="demo",
                     ...
                     logdir="", extra_epochs=0, verbose=0, normal_balanced=False, gaitset=False):
    """
    ...
    elif modality == 'of+gray':
        if use3D:
            input_shape1 = [(50, 60, 60), (25, 60, 60, 1)]
        else:
            input_shape1 = [(50, 60, 60), (25, 60, 60)]
        if gaitset:
            input_shape1 = [(25, 60, 60, 2), (25, 60, 60, 1)]
    else:
        if use3D:
    ...
    if previous_model != "":
        ...
        model = UWYHSemiNet_BothDatasets.build_or_load(input_shape, number_convolutional_layers,
                                   ...
                                   initnet=initnet, freeze_convs=freeze_convs, use3D=use3D, smoothlabels=smoothlabval,
                                          freeze_all=freeze_all, gaitset=gaitset)
...
#    plot_model(model, to_file=osp.join(experdir, 'model.png'))
    ...
    if datatype == 2:
        ...
        if nclasses == 155:
            ...
            if modality == 'of+gray':
                infofile = [
                    "/localscratch/users/fcastro/data/tumgaid_info/tfimdb_tum_gaid_N150_train_of25_60x60_info_of+gray.h5",
                    "/localscratch/users/fcastro/data/CASIAB_tf/tfimdb_casia_b_N074_train_of25_60x60.h5"]
            else:
                infofile = ["/localscratch/users/fcastro/data/tumgaid_info/tfimdb_tum_gaid_N150_train_of25_60x60_info_of+gray.h5",
...
#    rec2lab = db.rec2lab
    ...
    if datatype == 1:
        ...
        if nclasses == 155:
            datadirs = [osp.join(dbbasedir, "tfimdb_tum_gaid_N150_train_of25_60x6"),
                        "/home/GAIT_local/CASIAB_tf/tfimdb_casia_b_N074_train_of25_60x60"]

        elif nclasses == 224:
            if modality == 'of+gray':
                datadirs = ["/localscratch/users/fcastro/data/TUM_GAID_tf/tfimdb_tum_gaid_N150_train_of25_60x60",
                            "/localscratch/users/fcastro/data/TUM_GAID_tf/tfimdb_tum_gaid_N150_train_gray25_60x60",
                            "/localscratch/users/fcastro/data/CASIAB_tf/tfimdb_casia_b_N074_train_of25_60x60",
                            "/localscratch/users/fcastro/data/CASIAB_tf/tfimdb_casia_b_N074_train_gray25_60x60"]
                nmods=2
            else:
    ...
    train_generator = DataGeneratorGaitMMUWYH(allRecords_single_tr+allRecords_single_tr+allRecords_single_tr,
                                          ...
                                          dim=input_shape, n_classes=nclasses, labmap=labmap, isDebug=mj_isDebugging(),
                                           ...
                                           gait=gait_tr+gait_tr+gait_tr,
                                              expand_level=3, use3D=use3D, nmods=nmods, buildGaits=buildGaits, gaitset=gaitset)

    val_generator = DataGeneratorGaitMMUWYH(allRecords_single_val, datadir=datadirs, batch_size=batchsize/2,
                                            dataset_source = dataset_source_val+dataset_source_val+dataset_source_val,
                                        dim=input_shape, n_classes=nclasses, labmap=labmap, isDebug=mj_isDebugging(),
                                            ntype=datatype,
                                           gait=gait_val, expand_level=2, use3D=use3D, augmentation_x=0, nmods=nmods, buildGaits=buildGaits, gaitset=gaitset)
    ...
    if extra_epochs > 0:
        ...
        train_generator = DataGeneratorGaitMMUWYH(allRecords_single_val+allRecords_single_tr, datadir=datadirs, batch_size=batchsize,
                                        dim=input_shape, dataset_source=dataset_source_val+dataset_source_tr,
                                                  n_classes=nclasses, labmap=labmap, isDebug=mj_isDebugging(),
                                        gait=gait_val+gait_tr, expand_level=3, use3D=use3D, nmods=nmods,
                                                  ntype=datatype, buildGaits=buildGaits)
    ...
    if tuning:
                    ...
                    with tf.summary.create_file_writer(run_dir).as_default():
                        ...
                        final_model, experdir, val_acc = trainUWYHGaitNet(datadir=datadir, dbbasedir=dbbasedir,
                                                                          experfix=prefix, lr=lr, dropout=dropout_rate,
                                                                           ...
                                                                           margin=margin,
                                                                           ndense_units=num_units, normal_balanced=normal_balanced)
        ...
        final_model, experdir, val_acc = trainUWYHGaitNet(datadir=datadir, dbbasedir=dbbasedir, experfix=prefix, lr=lr,
                                                          ...
                                                          softlabel=softlabel, datatype=datatype, infodir=infodir,
                                                           extra_epochs=extraepochs, verbose=verbose, normal_balanced=normal_balanced, gaitset=gaitset)
