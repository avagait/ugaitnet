...
from nets.mj_uwyhNets_ba import UWYHSemiNet, UWYHSemiNet3Mods
...
def mj_computeDistMetrics(model, val_generator, multitask, lab4color):
    ...
    for bix in range(len(val_generator)):
        ...
        codes0 = UWYHSemiNet.encode(model, [of0, gray0, depth0], [uof0, ug0, ud0])
        if multitask:
...
def trainUWYHGaitNet(datadir="matimdbtum_gaid_N150_of25_60x60_lite", dbbasedir='/home/GAIT_local/SSD', experfix="demo",
                     nclasses=0, lr=0.001, dropout=0.4,
                     ...
                     modality="gray", modality0='of',
                     initnet="", use3D=False, softlabel=False, aux_losses=False,
                     datatype = 1, infodir="", freeze_all=False, nofreeze=False,
                     with_missing=True, logdir="", extra_epochs=0, verbose=0):
    """
    ...
    if use3D:
        input_shape = [(50, 60, 60), (25, 60, 60, 1), (25, 60, 60, 1)]
    else:
    ...
    # if use3D:
    #     infix = "_"+ modality0 + "+" + modality + "3D" + infix
    # else:
    #     infix = "_"+ modality0 + "+" + modality + infix
    if not with_missing:
        infix = "_full"

    if use3D:
        infix = "_3mods_3D" + infix
    else:
        infix = "_3mods" + infix
    ...
    if previous_model != "":
        ...
        model = UWYHSemiNet3Mods.loadnet(previous_model)
    else:
        if initnet != "":
            print("* Model will be init from: "+initnet)

        model = UWYHSemiNet3Mods.build_or_load(input_shape, number_convolutional_layers, filters_size, filters_numbers,
                                               ndense_units, weight_decay, dropout, optimizer=optimfun, margin=margin,
                                               nclasses=nclasses, loss_weights=loss_weights, initnet=initnet,
                                               freeze_convs=freeze_convs, use3D=use3D, smoothlabels=smoothlabval,
                                               freeze_all=freeze_all, aux_losses=aux_losses)
...
#    plot_model(model, to_file=osp.join(experdir, 'model.png'))
    ...
    if logdir == "":
        logdir = experdir
        # Save checkpoint
        #chkptname = osp.join(logdir, "weights.{epoch:02d}-{val_loss:.2f}.hdf5")
...
#        Igray = dd.io.load(osp.join("tfimdb_tum_gaid_N{}_train_{}25_60x60.h5".format(nclasses, modality)))
    ...
    if datatype == 1:
        ...
        if nclasses == 155:
            datadirs = [osp.join(dbbasedir, "tfimdb_tum_gaid_N155_ft_of25_60x60"),
                        osp.join(dbbasedir, "tfimdb_tum_gaid_N155_ft_gray25_60x60"),
                        osp.join(dbbasedir, "tfimdb_tum_gaid_N155_ft_depth25_60x60")]
        else:
            datadirs = [osp.join(dbbasedir, "tfimdb_tum_gaid_N150_train_of25_60x60"),
                        osp.join(dbbasedir, "tfimdb_tum_gaid_N150_train_gray25_60x60"),
    ...
    train_generator = DataGeneratorGaitMMUWYH(allRecords_single_tr+allRecords_single_tr+allRecords_single_tr,
                                          ...
                                          dim=input_shape, n_classes=nclasses, labmap=labmap, isDebug=mj_isDebugging(),
                                              ntype=datatype,
                                              gait=gait_tr+gait_tr+gait_tr, nmods=3,
                                              expand_level=exp_lev_tr, use3D=use3D, aux_losses=aux_losses)
    ...
    val_generator = DataGeneratorGaitMMUWYH(allRecords_single_val, datadir=datadirs, batch_size=batchsize,
                                        dim=input_shape, n_classes=nclasses, labmap=labmap, isDebug=mj_isDebugging(),
                                            ntype=datatype,
                                            gait=gait_val, nmods=3,
                                            expand_level=exp_lev_val, use3D=use3D, augmentation_x=0, aux_losses=aux_losses)
    ...
    if ep_steps > 1:  # Save partial models in case the process dies
        ...
        for eix in range(ep_steps):
            ...
            model, hist = UWYHSemiNet.fit_generator(model, ecum+epochs_, callbacks, train_generator, val_generator,
                                                    ecum, None, None, nworkers=nworkers) #ecum, steps_per_epoch, validation_steps)
            #ecum += epochs_
            ecum += len(hist.epoch)
            if ecum % 5 == 0:
            ...
            if not use3D:
                mj_save_filters(model, osp.join(experdir, "metrics"), step=ecum + epochs_, branchname="depthBranch",
                                title="filters-depth")
            else:
                mj_save_filters3d(model, osp.join(experdir, "metrics"), step=ecum + epochs_, branchname="depthBranch",
                                title="filters-depth-3d")
            ...
            # TODO stop if accurary is almost perfect
    ...
    if extra_epochs > 0:
        ...
        train_generator = DataGeneratorGaitMMUWYH(allRecords_single_val+allRecords_single_tr, datadir=datadirs, batch_size=batchsize,
                                        dim=input_shape, n_classes=nclasses, labmap=labmap, isDebug=mj_isDebugging(),
                                        gait=gait_val+gait_tr, expand_level=3, use3D=use3D, nmods=3,
                                                  ntype=datatype, aux_losses=aux_losses)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--aux_losses', default=False, action='store_true', help="Add an auxiliar loss to each branch?")
    ...
    parser.add_argument('--mod0', type=str, required=False,
                        default="of",
                        help="Base modality: of, gray, depth")
    ...
    if tuning:
                    ...
                    with tf.summary.create_file_writer(run_dir).as_default():
                        ...
                        final_model, experdir, val_acc = trainUWYHGaitNet(datadir=datadir, dbbasedir=dbbasedir,
                                                                          experfix=prefix, lr=lr, dropout=dropout_rate,
                                                                           ...
                                                                           margin=margin,
                                                                           ndense_units=num_units, aux_losses=aux_losses)
        ...
        final_model, experdir, val_acc = trainUWYHGaitNet(datadir=datadir, dbbasedir=dbbasedir, experfix=prefix, lr=lr,
                                                           dropout=dropout, experdirbase=experdirbase,
                                                           ...
                                                           modality=modality, modality0=modality0,
                                                          initnet=initnet, use3D=use3D, freeze_all=freeze_all,
                                                          softlabel=softlabel, datatype=datatype, infodir=infodir,
                                                          with_missing=with_missing, nofreeze=nofreeze, aux_losses=aux_losses,
