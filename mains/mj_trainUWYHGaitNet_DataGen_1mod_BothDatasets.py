...
#from nets import BasicGaitNet, TrainValTensorBoard
import deepdish as dd
from nets.mj_uwyhNets_bothDatasets import UWYHSemiNet_BothDatasets
from nets.mj_metrics import mj_eerVerifDist
from tensorflow.keras.callbacks import ReduceLROnPlateau

from data.mj_dataGeneratorMMUWYHBothDatasets import DataGeneratorGaitMMUWYH
...
def mj_splitTrainValGaitByInfo(Iof):
    allPairs = []
    for i in Iof['records']:
        allPairs.append((i[0][0], i[1]))

    rec2gait = Iof["rec2gait"]
    rec2vid = Iof["rec2vid"]

    # Split in training/val data, grouped per video
    rec2lab = {p[0]: p[1] for p in allPairs}
    all_keys = [p[0] for p in allPairs]  # rec2vid.keys()
...
#-------------------------------------------------------------
def mj_splitTrainValGaitByInfo_casia(Iof):
    allPairs = []
    rec2gait_casia = {}
    for i in range(len(Iof['file'])):
        allPairs.append((Iof['file'][i], Iof['label'][i]))
        rec2gait_casia[Iof['file'][i]] = Iof['gait'][i]

    rec2gait = rec2gait_casia
    #rec2vid = Iof["rec2vid"]

    # Split in training/val data, grouped per video
    all_vids = Iof['videoId']
    uvids = np.unique(all_vids)
    nvids = len(uvids)
    np.random.shuffle(uvids)
    perc = 0.09
...
def mj_splitTrainValGaitByInfo_BothDatasets(Iof_tum, Iof_casia):
    allPairs_tum = []
    allPairs_casia = []
    rec2gait_casia = {}
    dataset_source_all = []
    ...
    for i in Iof_tum['records']:
        allPairs_tum.append((i[0][0], i[1]))

    for i in range(len(Iof_casia['file'])):
        allPairs_casia.append((Iof_casia['file'][i], Iof_casia['label'][i] + 305))
        rec2gait_casia[Iof_casia['file'][i]] = Iof_casia['gait'][i] + 3

    allPairs = allPairs_tum.copy()
    allPairs.extend(allPairs_casia)

    dataset_source_all = [0] * len(allPairs_tum)
    dataset_source_all.extend([1]*len(allPairs_casia))


    rec2gait_tum = Iof_tum["rec2gait"] #+ Iof_casia['gait']
    rec2gait_all = rec2gait_tum.copy()
    rec2gait_all.update(rec2gait_casia)

    rec2vid = Iof_tum["rec2vid"]

    # Split in training/val data, grouped per video
    rec2lab = {p[0]: p[1] for p in allPairs_tum}
    all_keys = [p[0] for p in allPairs_tum]  # rec2vid.keys()
    all_vids_tum = [rec2vid[key] for key in all_keys]
    uvids = np.unique(all_vids_tum)

    all_vids_casia = Iof_casia['videoId']+max(uvids)
    all_vids_all = all_vids_tum
    all_vids_all.extend(all_vids_casia)

    uvids = np.unique(all_vids_all)
    nvids = len(uvids)
    np.random.shuffle(uvids)
    perc = 0.09
    nval = int(perc * nvids)
    ...
    dataset_source_tr = []
    for vix in vids_tr:
        idx = np.where(all_vids_all == vix)[0]
        this_vid_pairs = [allPairs[ix] for ix in idx]  # [all_keys[ix] for ix in idx]
        this_souce_pairs = [dataset_source_all[ix] for ix in idx]
        dataset_source_tr = dataset_source_tr + this_souce_pairs
        allRecords_single_tr = allRecords_single_tr + this_vid_pairs
    ...
    for vix in vids_val:
        ...
        this_vid_pairs = [allPairs[ix] for ix in idx]  # [all_keys[ix] for ix in idx]
        this_souce_pairs = [dataset_source_all[ix] for ix in idx]
        dataset_source_val = dataset_source_val + this_souce_pairs
        allRecords_single_val = allRecords_single_val + this_vid_pairs

    return allRecords_single_tr, allRecords_single_val, rec2gait_all, dataset_source_tr, dataset_source_val
...
#        codes0 = UWYHSemiNet_BothDatasets.encode(model, of0, gaitset)
        seq_of = model.get_layer('flatten')
        mod = tf.keras.Model(inputs=model.input, outputs=seq_of.output)
        of_codes = mod(of0)

        # L2 normalize embeddings
        codes_norm_tf = tf.keras.layers.Lambda(lambda x: tf.math.l2_normalize(x, axis=1))(of_codes)

        # Get the numpy matrix
        codes0 = codes_norm_tf.numpy()
...
def trainUWYHGaitNet(datadir="matimdbtum_gaid_N150_of25_60x60_lite", dbbasedir='/home/GAIT_local/SSD', experfix="demo",
                     ...
                     modality="of",
                     initnet="", use3D=False, softlabel=False, softlabelvalue=0.1,
                     datatype = 1, infodir="", freeze_all=False, nofreeze=False,
                     logdir="", extra_epochs=0, verbose=0, focal_loss=False, normal_balanced=False, only_triplet=False,
                     normalize_data=False, gaitset=False):
    """
    ...
    if softlabel:
        infix = infix + "_sl{:.1f}".format(softlabelvalue)
        smoothlabval = softlabelvalue
    else:
        smoothlabval = 0
    ...
    if focal_loss:
        infix = infix + "_focalLoss"

    if normal_balanced:
        infix = infix + "_normalBalanced"

    if only_triplet:
        infix = infix + "_onlyTriplet"

    if gaitset:
        infix = infix + 'gaitset'

    if normalize_data:
        infix = infix + "_normalizeData"
        normalize_paths = ["/home/GAIT_local/SSD/tfimdb_tum_gaid_N150_train_of25_60x60_mean_std.h5",
                           "/home/GAIT_local/SSD/tfimdb_casia_b_N074_train_of25_60x60_mean_std.h5"]
    else:
    ...
    if previous_model != "":
        ...
        model = UWYHSemiNet_BothDatasets.loadnet(previous_model)
    else:
        if initnet != "":
            print("* Model will be init from: "+initnet)

        model = UWYHSemiNet_BothDatasets.build_or_load(input_shape, number_convolutional_layers,
                                   ...
                                   freeze_all=freeze_all, focal_loss=focal_loss, only_triplet=only_triplet, gaitset=gaitset)
...
#    plot_model(model, to_file=osp.join(experdir, 'model.png'))
    ...
    if datatype == 2:
        if infodir == "":
            ...
            h5dir = infodir
        if nclasses == 150:
            infofile = osp.join(h5dir, "tfimdb_tum_gaid_N150_train_of25_60x60_info_of+gray.h5".format(modality))
        elif nclasses == 224:
            infofile = ["/mnt/scratch/users/ac/fcastro/experiments/tumgaid_info/tfimdb_tum_gaid_N150_train_of25_60x60_info_of+gray.h5",
                        "/localscratch/users/fcastro/data/CASIAB_tf/tfimdb_casia_b_N074_train_{}25_60x60.h5".format(modality)]
        elif nclasses == 74:
            infofile = osp.join(h5dir, "/home/GAIT_local/CASIAB_tf/tfimdb_casia_b_N074_train_{}25_60x60.h5".format(modality))
        else:
            ...
            if modality == "depth":
                ...
                infofile = osp.join(h5dir, "tfimdb_tum_gaid_N150_train_of25_60x60_info_of+gray.h5")
        if nclasses == 150:
...
#        Igray = dd.io.load(osp.join("tfimdb_tum_gaid_N{}_train_{}25_60x60.h5".format(nclasses, modality)))
            ...
            dataset_source_tr = [0] * len(allRecords_single_tr)
            dataset_source_val = [0] * len(allRecords_single_val)
        elif nclasses == 74:
            Iof = dd.io.load(infofile)
            #        Igray = dd.io.load(osp.join("tfimdb_tum_gaid_N{}_train_{}25_60x60.h5".format(nclasses, modality)))

            allRecords_single_tr, allRecords_single_val, rec2gait = mj_splitTrainValGaitByInfo_casia(Iof)
            dataset_source_tr = [0] * len(allRecords_single_tr)
            dataset_source_val = [0] * len(allRecords_single_val)
        else:
            Iof_tum = dd.io.load(infofile[0])
            Iof_casia = dd.io.load(infofile[1])
            allRecords_single_tr, allRecords_single_val, rec2gait, dataset_source_tr, dataset_source_val = mj_splitTrainValGaitByInfo_BothDatasets(Iof_tum, Iof_casia)
    ...
    if datatype == 1:
        ...
        gait_tr = [rec2gait[r[0]] for r in allRecords_single_tr]
        gait_val = [rec2gait[r[0]] for r in allRecords_single_val]
    ...
    if datatype == 1:
        if nclasses == 150:
            datadirs = [osp.join(dbbasedir, "matimdbtum_gaid_N150_{}25_60x60".format(modality)),
                        osp.join(dbbasedir, "matimdbtum_gaid_N150_{}25_60x60".format(modality))]
        elif nclasses == 74:
            datadirs = [osp.join(dbbasedir, "tfimdb_casia_b_N074_train_{}25_60x60".format(modality)),
        ...
        if nclasses == 150:
            datadirs = [osp.join(dbbasedir, "tfimdb_tum_gaid_N150_train_{}25_60x6".format(modality)),
                        osp.join(dbbasedir, "tfimdb_tum_gaid_N150_ft_{}25_60x60".format(modality))]
            ...
            datadirs = ["/localscratch/users/fcastro/data/TUM_GAID_tf/tfimdb_tum_gaid_N150_train_{}25_60x60".format(modality),
                          "/localscratch/users/fcastro/data/CASIAB_tf/tfimdb_casia_b_N074_train_{}25_60x60".format(modality)]

        else:
            datadirs = [osp.join(dbbasedir, "tfimdb_tum_gaid_N150_train_{}25_60x60".format(modality)),
    ...
    train_generator = DataGeneratorGaitMMUWYH(allRecords_single_tr+allRecords_single_tr+allRecords_single_tr,
                                              ...
                                              dataset_source=dataset_source_tr+dataset_source_tr+dataset_source_tr,
                                          dim=input_shape, n_classes=nclasses, labmap=labmap, isDebug=IS_DEBUG,
                                              ntype=datatype,
                                           gait=gait_tr+gait_tr+gait_tr,
                                              expand_level=3, use3D=use3D, nmods=1, focal_loss= focal_loss, buildGaits=buildGaits,
                                              normalize_paths=normalize_paths, gaitset=gaitset)

    val_generator = DataGeneratorGaitMMUWYH(allRecords_single_val, datadir=datadirs, batch_size=batchsize,
                                            dataset_source = dataset_source_val+dataset_source_val+dataset_source_val,
                                        dim=input_shape, n_classes=nclasses, labmap=labmap, isDebug=IS_DEBUG,
                                            ntype=datatype,
                                           gait=gait_val, expand_level=2, use3D=use3D, augmentation_x=0, nmods=1,
                                            focal_loss= focal_loss, buildGaits=buildGaits,normalize_paths=normalize_paths, gaitset=gaitset)
    ...
    if ep_steps > 1: # Save partial models in case the process dies
        ...
        for eix in range(ep_steps):
            ...
            if verbose > 1:
                print(experdir)
            model, hist = UWYHSemiNet_BothDatasets.fit_generator(model, ecum+epochs_, callbacks, train_generator, val_generator,
            ...
            distances, eer_val, chance, data_metric = mj_computeDistMetrics(model, val_generator, multitask, lab4color, gaitset)
        ...
        model, hist = UWYHSemiNet_BothDatasets.fit_generator(model, epochs, callbacks, train_generator, val_generator,
    ...
    if extra_epochs > 0:
        ...
        train_generator = DataGeneratorGaitMMUWYH(allRecords_single_val+allRecords_single_tr, datadir=datadirs, batch_size=batchsize,
                                        dim=input_shape, dataset_source=dataset_source_val+dataset_source_tr,
                                        n_classes=nclasses, labmap=labmap, isDebug=IS_DEBUG,
                                        gait=gait_val+gait_tr, expand_level=3, use3D=use3D, nmods=1, focal_loss= focal_loss,
                                                  ntype=datatype, buildGaits=buildGaits,normalize_paths=normalize_paths)
        ...
        model, hist = UWYHSemiNet_BothDatasets.fit_generator(model, ft_epochs, callbacks, train_generator, val_generator,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--softlabelvalue', required=False, type=float, default=0.1, help="Soft labels value")
    parser.add_argument('--freezeall', default=False, action='store_true', help="Freeze all weights?")
    parser.add_argument('--focalloss', default=False, action='store_true', help="Focal loss?")
    parser.add_argument('--normalbalanced', default=False, action='store_true', help="Normal balanced?")
    parser.add_argument('--gaitset', default=False, action='store_true')
    parser.add_argument('--onlytriplet', default=False, action='store_true', help="Only triplet?")
    parser.add_argument('--normalizedata', default=False, action='store_true', help="Normalice data?")
    ...
    softlabelvalue = args.softlabelvalue
    IS_DEBUG = args.debug
    hardnegs = args.hn
    freeze_all = args.freezeall
    focal_loss = args.focalloss
    normal_balanced = args.normalbalanced
    normalize_data = args.normalizedata
    nofreeze = args.nofreeze
    only_triplet = args.onlytriplet
    ...
    if tuning:
                    ...
                    with tf.summary.create_file_writer(run_dir).as_default():
                        ...
                        final_model, experdir, val_acc = trainUWYHGaitNet(datadir=datadir, dbbasedir=dbbasedir,
                                                                          experfix=prefix, lr=lr, dropout=dropout_rate,
                                                                           ...
                                                                           margin=margin,
                                                                           ndense_units=num_units, focal_loss=focal_loss,
                                                                          normal_balanced=normal_balanced, only_triplet=only_triplet,
        ...
        final_model, experdir, val_acc = trainUWYHGaitNet(datadir=datadir, dbbasedir=dbbasedir, experfix=prefix, lr=lr,
                                                          ...
                                                          nofreeze=nofreeze,
                                                          softlabel=softlabel, softlabelvalue=softlabelvalue ,datatype=datatype, infodir=infodir,
                                                           extra_epochs=extraepochs, verbose=verbose, focal_loss=focal_loss,
                                                          normal_balanced=normal_balanced, only_triplet=only_triplet, normalize_data=normalize_data, gaitset=gaitset)
