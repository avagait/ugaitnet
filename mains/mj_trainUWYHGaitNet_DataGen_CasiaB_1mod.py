...
def trainUWYHGaitNet(datadir="tfimdb_casia_b_N074_train_of25_60x60", dbbasedir='/home/GAIT_local/SSD', experfix="demo",
                     ...
                     postriplet=1,
                     logdir="", extra_epochs=0, gaitset=False, repetitions=16, verbose=0):
    """
    ...
    if previous_model != "":
        ...
        model = UWYHSemiNet.build_or_load(input_shape, number_convolutional_layers,
                                   ...
                                   initnet=initnet, freeze_convs=freeze_convs, use3D=use3D, smoothlabels=smoothlabval,
                                          freeze_all=freeze_all, postriplet=postriplet, gaitset=gaitset)
...
#    plot_model(model, to_file=osp.join(experdir, 'model.png'))
    ...
    if logdir == "":
        ...
        callbacks = [tensorboard, checkpoint, es_callback]
    ...
    if nclasses == 74 or nclasses==0:
        infofile = osp.join(h5dir, "tfimdb_casia_b_N074_train_of25_60x60_info_of+gray.h5")
    else:
        infofile = osp.join(h5dir, "tfimdb_casia_b_N050_ft_of25_60x60_info_of+gray.h5")
    ...
    if nclasses == 74 or nclasses == 0:
        ...
        datadirs[0] = osp.join(dbbasedir, "tfimdb_casia_b_N074_train_{}25_60x60".format(modality))
    else:
        datadirs = [osp.join(dbbasedir, "tfimdb_casia_b_N050_ft_{}25_60x60".format(modality)),
                    osp.join(dbbasedir, "tfimdb_casia_b_N050_ft_{}25_60x60".format(modality))]

    # MJ: augment training data
    print(datadirs, flush=True)
    train_generator = DataGeneratorGaitMMUWYH(allRecords_single_tr, datadir=datadirs, batch_size=batchsize,
                                          dim=input_shape, n_classes=nclasses, labmap=labmap, isDebug=mj_isDebugging(),
                                              ntype=datatype,
                                           gait=gait_tr, expand_level=3, use3D=use3D, nmods=1, repetition=repetitions, gaitset=gaitset)

    val_generator = DataGeneratorGaitMMUWYH(allRecords_single_val, datadir=datadirs, batch_size=batchsize,
                                        dim=input_shape, n_classes=nclasses, labmap=labmap, isDebug=mj_isDebugging(),
                                            ntype=datatype,
                                           gait=gait_val, expand_level=2, use3D=use3D, augmentation_x=0, nmods=1, repetition=repetitions, gaitset=gaitset)
    ...
    if ep_steps > 1: # Save partial models in case the process dies
        ...
        model, hist = UWYHSemiNet.fit_generator(model, epochs, callbacks, train_generator, None,
                                                 initepoch, train_generator.__len__(), None)
    ...
    if extra_epochs > 0:
        ...
        train_generator = DataGeneratorGaitMMUWYH(allRecords_single_val+allRecords_single_tr, datadir=datadirs, batch_size=batchsize,
                                        ...
                                        gait=gait_val+gait_tr, expand_level=3, use3D=use3D,
                                                  ntype=datatype, nmods=1, repetitions=repetitions, gaitset=gaitset)
        ...
        model, hist = UWYHSemiNet.fit_generator(model, ft_epochs, callbacks, train_generator, None,
    ...
    if tuning:
        ...
        final_model, experdir, val_acc = trainUWYHGaitNet(datadir=datadir, dbbasedir=dbbasedir, experfix=prefix, lr=lr,
                                                          ...
                                                          postriplet=postriplet,
                                                           extra_epochs=extraepochs, repetitions=repetitions, gaitset=gaitset, verbose=verbose)
